...
## Этап 5: Страница Store

1. Создай файл `src/pages/store/index.astro`.
2. В нём импортируй Astro.glob для коллекции `src/content/projects/*.md` и отфильтруй посты, у которых `purchasable: true`.
3. Создай React-остров `StoreIsland.jsx` (в `src/components/`), который получит массив отфильтрованных постов.
4. В `StoreIsland`:
   - Используй `SearchBar` для поиска по названию и автору.
   - Используй `FilterPanel` для фильтрации по тегам (как в архивах).
   - Отображай посты **сеткой** через компонент `GridCard`. Каждая карточка показывает:
     - Обложку (первое фото из gallery или цветной блок с инициалами, если фото нет).
     - Название.
     - Автора.
     - Кнопку "View details" или кликабельность всей карточки.
   - При клике на карточку — открывай `DetailPanel` с содержимым поста (так же, как в `ArchiveIsland`: галерея, текст, блок «Contact author» на самом видном месте).
5. Скрой предзагруженные markdown-данные в скрытые div'ы аналогично `ArchiveIsland`, чтобы не делать лишних запросов.
6. Добавь в `global.css` стили для сетки карточек (`.store-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(240px, 1fr)); gap: 2rem; }` и т.п.).
7. Убедись, что на мобильном карточки адаптируются (одна колонка).
8. Страница `/store` должна быть доступна, меню обновится автоматически (если ещё нет пункта Store, добавь в навигацию).
...
Делаем главную страницу (/) по принципам из PROJECT-PROTOCOL.md.

ЗАДАЧА
Создать лендинг-секцию поверх текущей сетки, которая будет меняться каждый
семестр (осень / весна / лето).

1. Создай конфиг src/config/seasons.ts. Три записи:
   - fall:   тема "Material" (объекты: больше шрифтов, цвета холодные)
   - spring: тема "Exhibition" (объекты: проекты + шрифты, цвета тёплые)
   - summer: тема "Archive Rest" (объекты: один проект + один шрифт, спокойно)
   Авто-определение текущего сезона по месяцу (9-1 = осень, 2-6 = весна, 7-8 = лето).

2. Главная (src/pages/index.astro):
   - Импортирует сезон, получает случайные объекты из коллекций (Typefaces, Projects, Journal, Store).
   - Визуал:
     - 2 ряда горизонтального скролла с разной скоростью (parallax-like).
     - Карточки шрифтов: название набрано живым шрифтом (крупно, OTF), без теней, фон белый/тёмный.
     - Карточки проектов: первая картинка из gallery + название системным шрифтом.
   - Анимация: бесконечная прокрутка, CSS transform: translateX().
   - Мобилка: вертикальный стэк карточек, без скролла.

3. Добавь в глобальные стили .hero-section, .marquee-row, .marquee-card.
   Никаких картинок-заглушек. Минимум анимаций, только transform.
